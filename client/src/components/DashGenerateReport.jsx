...
export default function DashGenerateReport() {
  const getLastSunday = () => {
  ...
  const selectedReports = reports
    .filter((report) => {
      ...
      const matchesSearch =
        report?.reporter?.surname?.toLowerCase().includes(search) ||
        report?.reporter?.othernames?.toLowerCase().includes(search) ||
  ...
  return (
    <div className="w-full table-auto overflow-x-scroll md:mt-4 md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500 flex gap-5 mt-8 sm:mt-0">
      ...
      <div className="flex flex-col gap-5 w-full">
        <h1 className="text-xl font-extrabold">Generate Report:</h1>
        ...
        >
          {summary?.length > 0 ? (
            <div className="flex flex-col bg-white p-2 rounded-md w-full lg:w-1/2 mx-auto">
              <h1 className="text-lg font-bold">Report Summary</h1>
              <p className="text-sm font-semibold mb-3">
                {startDate} to {endDate}
              </p>

              <h2 className="font-semibold text-sm">Total Reports:</h2>
              {summary.map((item) => (
                <div key={item.name}>
                  <p className="flex gap-1">
                    {item.name}: {item.totalReports}
                  </p>
                </div>
              ))}
              ...
              {summary.map((item) => (
                <div key={item.name}>
                  <p className="flex gap-1">
                    {item.name}: {item.interventions}
                  </p>
                </div>
              ))}

              {/* excel file download link */}
              <div className="flex items-center gap-1 px-3 py-1 bg-blue-700 text-white mt-5 w-fit rounded hover:scale-105 hover:bg-blue-900 transition-all duration-300 text-sm">
                <FileDown size={16} />
                {excelFile && <a href={excelFile}>Download Excel</a>}
              </div>
            </div>
